/**
 * Clean logger utility for Cloudflare Worker environment.
 * Strips emojis, formats log tags, and redacts sensitive data in production.
 */
...
function formatTag(tag: string): string {
  ...
  return cleanTag ? `[${cleanTag}]` : "[Worker]";
}

export const workerLogger = {
  log(tag: string, message: string, envName?: string, ...args: unknown[]) {
    const isProd = envName === "production";
    if (isProd) return; // Suppress verbose log output in production worker
  ...
  info(tag: string, message: string, envName?: string, ...args: unknown[]) {
    const isProd = envName === "production";
    const cleanMsg = stripEmojis(message);
  ...
  warn(tag: string, message: string, envName?: string, ...args: unknown[]) {
    const isProd = envName === "production";
    const cleanMsg = stripEmojis(message);
    const cleanArgs = args.map((arg) => sanitizeValue(arg, isProd));
  ...
  error(tag: string, message: string, envName?: string, ...args: unknown[]) {
    const isProd = envName === "production";
    const cleanMsg = stripEmojis(message);
    const cleanArgs = args.map((arg) => sanitizeValue(arg, isProd));
