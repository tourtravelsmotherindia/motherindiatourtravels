  ...
  return (
    <AdminCard>
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="text-black hover:text-neutral-600 transition-colors p-1 rounded-lg hover:bg-neutral-50 cursor-pointer">
          <MoreVertical className="w-3.5 h-3.5" />
        </button>
      </div>

      <div className="flex items-end justify-between">
        <h3 className="text-3xl font-display font-extrabold text-black leading-none tracking-tight">
          {value}
        </h3>
        {change && (
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-xl md:text-2xl font-bold font-display text-black">{getGreeting()}</h2>
          <p className="text-sm text-black font-medium mt-0.5">
            Here&apos;s a snapshot of what&apos;s happening with the travel portal today.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-black bg-white border border-neutral-100 rounded-full px-4 py-2 font-semibold shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <AdminCard noPadding className="xl:col-span-2 overflow-hidden">
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-neutral-50">
            <div>
              <h3 className="text-sm font-bold font-display text-black">Recent Inquiries</h3>
              <p className="text-xs text-black mt-0.5">Last 5 booking inquiries received.</p>
            </div>
            <Link
              href="/manage/bookings/"
          ...
          <div className="overflow-x-auto no-scrollbar">
            {loading ? (
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="text-black font-extrabold uppercase tracking-wider text-[10px]">
                  ...
                  {recentBookings.map((b) => (
                    <tr key={b.id} className="hover:bg-neutral-50/60 transition-colors">
                      <td className="py-3.5 px-6">
                        ...
                        <p className="text-[10px] text-black font-normal mt-0.5">{b.email}</p>
                      </td>
                      <td className="py-3.5 px-6 text-black">
                        {mounted ? formatLocalDateTime(b.createdAt) : "..."}
                      </td>
                      <td className="py-3.5 px-6 text-black font-semibold">
                        {b.numberOfPeople || b.adults + b.children + b.infants}
                      </td>
                      <td className="py-3.5 px-6 text-black">{b.rooms} room(s)</td>
        ...
        <div className="space-y-4">
          {/* Quick Actions */}
          <AdminCard>
            <h3 className="text-sm font-bold font-display text-black mb-4">Quick Actions</h3>
              ...
              {quickShortcuts.map((sc, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-bold text-black group-hover:text-brand transition-colors">
                      {sc.title}
                    </h4>
                    <p className="text-[10px] text-black mt-0.5 font-medium truncate">{sc.desc}</p>
                  </div>
          ...
          <AdminCard>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-bold font-display text-black">System Health</h3>
                <p className="text-[10px] text-black mt-0.5 font-medium">Real-time diagnostics</p>
              </div>
              <button
