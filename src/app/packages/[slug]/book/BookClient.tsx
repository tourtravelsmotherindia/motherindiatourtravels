...
export default function BookClient({ packageData, allPackages, companyData }: BookClientProps) {
  ...
  const tourStyle = useMemo(() => {
    ...
    return "Classic";
  }, [packageData.category_ids]);

  const activeVariant = useMemo(() => {
    if (packageData.variants && packageData.variants.length > 0) {
      return packageData.variants[0];
    }
    return {
      days: packageData.min_days || 5,
      nights: packageData.min_nights || 4,
      duration_text:
        packageData.duration_range || `${packageData.min_nights}N/${packageData.min_days}D`,
    };
  }, [packageData]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      showToast(
        ...
        `We have received your booking request for "${pkgName}". Our tour expert will contact you shortly.`,
      );
      setFullName("");
      setEmailAddress("");
