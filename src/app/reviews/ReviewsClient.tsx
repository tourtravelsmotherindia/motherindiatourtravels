...
export default function ReviewsClient({
  ...
  const sources = ["All", "Google", "TripAdvisor"];

  // Filter reviews based on search query and source platform
  ...
  return (
    <PageShell companyData={companyData} bgClass="bg-white">
      <div className="pt-12 md:pt-20 pb-section-loose font-sans">
        {/* Header Block consistent with FAQs, About and Policy pages */}
        ...
        <div className="layout-container mb-10">
          <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            ...
            <div className="flex gap-2 p-1 bg-neutral-100/80 rounded-2xl shrink-0">
              {sources.map((src) => {
                const isActive = selectedSource === src;
                const label = src === "All" ? "All Reviews" : `${src} Reviews`;
                return (
                  <button
                    key={src}
                    onClick={() => setSelectedSource(src)}
                    className={`py-2 px-5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                      isActive
                        ? "bg-white text-black shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
                        : "text-neutral-600 hover:text-black hover:bg-white/40"
                    }`}
                  >
                    {label}
                  </button>
        ...
        {/* Masonry-like dynamic reviews list */}
          ...
          {filteredReviews.length > 0 ? (
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6 [column-fill:balance]">
              {filteredReviews.map((review) => {
                ...
                const cardContent = (
                  <div className="break-inside-avoid relative bg-white border border-border-light rounded-2xl p-6 hover:shadow-[0_4px_20px_rgba(0,0,0,0.05)] hover:border-neutral-300 transition-all duration-300 flex flex-col gap-4">
                    {/* Header: Profile image, reviewer name & source badge */}
                ...
                if (review.reviewLink) {
                  return (
                    <a
                      ...
                      className="block no-underline outline-none break-inside-avoid cursor-pointer mb-6"
                    >
                      {cardContent}
                    </a>
                  );
                ...
                return (
                  <div key={review.id} className="block break-inside-avoid mb-6">
                    {cardContent}
                  </div>
                );
              })}
            ...
            <div className="text-center py-20 bg-neutral-50 border border-dashed border-neutral-200 rounded-3xl">
              <p className="text-lg text-neutral-500 font-medium">
                No traveler reviews matching your filters. Try checking other terms!
              </p>
            </div>
          )}
        </div>
