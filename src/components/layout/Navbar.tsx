...
export default function Navbar({ transparent = false }: { transparent?: boolean }) {
  ...
  return (
    <header
      className={`fixed left-0 right-0 z-50 transition-all duration-500 ${scrolled || !isTransparentPage
        ? "top-3 md:top-4 layout-container"
        : "top-0 w-full px-6 md:px-12 lg:px-24 bg-gradient-to-b from-black/40 to-transparent"
        }`}
    >
      <nav
        className={`transition-all duration-500 flex items-center justify-between ${scrolled || !isTransparentPage
      ...
      >
        <Link href="/" className="flex items-center gap-3 group shrink-0 select-none">
          ...
          <div className="flex flex-col notranslate">
            <span
              className={`block font-display text-[10px] sm:text-xs lg:text-sm font-bold tracking-[0.18em] uppercase leading-none transition-colors duration-300 ${!scrolled && isTransparentPage ? "text-white" : "text-foreground"
                }`}
            >
              Mother India
            </span>
            <span
              className={`block font-sans text-[7px] sm:text-[8px] lg:text-[9px] font-semibold tracking-[0.25em] uppercase leading-none mt-1 transition-colors duration-300 ${!scrolled && isTransparentPage ? "text-white/70" : "text-muted"
                }`}
            >
              Tour Travels
            </span>
          ...
          {navLinks.map((link) => {
            if (link.submenu) {
              return (
                <div key={link.name} className="relative group py-2">
                  <button
                    className={`flex items-center gap-1.5 font-semibold text-sm transition-colors duration-200 cursor-pointer select-none ${!scrolled && isTransparentPage
                  ...
                  >
                    <span>{link.name}</span>
                    <ChevronDown
                      className={`w-3.5 h-3.5 transition-transform duration-200 group-hover:rotate-180 ${!scrolled && isTransparentPage
            ...
            return (
              <Link
                ...
                className={`font-semibold text-sm transition-colors duration-200 ${!scrolled && isTransparentPage
                  ? "text-white hover:text-white/70"
                  : "text-foreground hover:text-brand"
                  }`}
              >
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <Link
            href="/search"
            className={`p-2 sm:p-2.5 rounded-full border transition-all duration-300 flex items-center justify-center cursor-pointer ${!scrolled && isTransparentPage
          ...
          <div className="hidden md:block">
            <Dropdown
              ...
              align="right"
              triggerClassName={`px-4 py-2 border transition-all duration-200 text-xs font-semibold w-auto shadow-none bg-transparent ${!scrolled && isTransparentPage
                ? "border-white/20 text-white hover:bg-white/10 hover:border-white/40"
          ...
          <button
            type="button"
            onClick={toggleMenu}
            className={`md:hidden p-2 rounded-full transition-colors duration-200 ${!scrolled && isTransparentPage
                  ...
                  return (
                    <div key={link.name} className="flex flex-col">
                      ...
                      >
                        <span>{link.name}</span>
                        <ChevronDown
                          className={`w-4 h-4 text-neutral-400 transition-transform duration-200 ${isSubmenuOpen ? "rotate-180" : ""
                            }`}
                        />
                      </button>
